...
<div class="card-header-left">
    ...
    <i class="fa fa-pencil-square-o" aria-hidden="true"></i>&nbsp;{{ __('homepage.candidate') }}
    </span>
</div>
<div class="card-header-right">
    <a id="export-excel" class="btn btn-success btn-sm" href="{{ route('excel.candidates') }}">Export to Excel</a>
</div>
@endsection
        ...
        <table id="datatable_serverside" class="table table-bordered table-sm table-striped table-vcenter table-ellipsis">
            <thead>
                <tr>
                    ...
                    <th style="width: 180px;">{{ __('homepage.status') }}</th>
        ...
        table = $('#datatable_serverside').DataTable({
            ...
            columns: [
                ...
                {data: 'vendor_name', sortable: true, className: '',
                    render: function (data, type, row, meta) {
                        return `
                            <a href="{{ route('candidate.list') }}/profile/${row.id}" type="submit" class="btn btn-sm btn-link" style="text-align:justify" data-toggle="tooltip" title="Show Profile">
                                ${row.vendor_name}
                            </a>
                        `;
                    }
