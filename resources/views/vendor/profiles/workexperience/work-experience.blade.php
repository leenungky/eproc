...
@php
    $formName = 'frmexperience';
    $formLayout = 'vendor.profiles.form.form_experience';
@endphp
...
require(['moment'], function(){
    require(["jquery", "bootstrap", "bootstrap-util", "metisMenu", "bootstrap-fileinput", "datetimepicker"], function () {
        require(["bootstrap-fileinput-fas"],function(){
            // with plugin options
            let maxUploadSize = parseInt(`{{ config('eproc.upload_file_size') }}`);
            $("#bast_wan_attachment").fileinput({'showUpload':false, 'previewFileType':'any','theme':'fas', 'maxFileSize' : maxUploadSize});
            $(".date").datetimepicker({
                useCurrent: false,
                format: uiDateFormat
            });
            $("#valid_from_date").on("change.datetimepicker", function (e) {
                $('#valid_thru_date').val("");
