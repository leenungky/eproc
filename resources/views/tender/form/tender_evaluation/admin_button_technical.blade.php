@if($statusProcess == 'registration-')
    <button class="btn btn-success btn_start_tc" @if(!$technical['canStart']) disabled @endif>
...
@elseif($statusProcess == 'started-3' && (count($workflowValues) > 1 && (int)$workflowValues[1] == 3))
    <button class="btn btn-success btn_open_tc" data-action="{{$workflowValues[1]}}" @if(!$technical['canOpen']) disabled @endif>
...
@elseif($statusProcess == 'opened-3' && (count($workflowValues) > 1 && (int)$workflowValues[1] == 3))
    <button class="btn btn-warning ml-2 btn_request_resubmission" @if(!$technical['canOpen']) disabled @endif>
        <i class="fa fa-undo-alt"></i>&nbsp;&nbsp;&nbsp;{{__('tender.process.request_resubmission_tc')}}</button>
    <button class="btn btn-success ml-2 btn_finish_tab" @if(!$technical['canFinish']) disabled @endif>
