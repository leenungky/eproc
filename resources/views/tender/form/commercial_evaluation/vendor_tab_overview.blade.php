<div class="tab-body">
    <div class="has-footer has-tab" style="padding: 0">
        ...
        @elseif(in_array($statusProcess , ['started-4','started-3']))
        <div class="alert alert-info alert-flat no-margin" role="alert" @if($tender->visibility_bid_document != 'PUBLIC') hidden @endif>
            <p>{{__('tender.process.info_pre_com2')}}</p>
        </div>
        @elseif(in_array($statusProcess , ['opened-3','opened-4']))
        <div class="alert alert-info alert-flat no-margin" role="alert" @if($tender->visibility_bid_document != 'PUBLIC') hidden @endif>
        ...
        <div id="card-schedule" class="card" style="margin-bottom: 20px;">
            <div class="card-header">
                <div class="card-header-left">
                    <span class="heading-title">{{__('tender.process_commercial_evaluation')}}</span>
                </div>
            </div>
            <div class="card-body card-schedule" style="padding-top: 20px;">
                <table class="table table-borderless table-sm">
                    <tr>
                        <th class="text-right" style="width: 25%">{{__('tender.schedule.fields.start_date', ['type' => __('tender.schedule_type_label.commercial')])}}</th>
                    ...
                    <tr>
                        <th class="text-right" style="width: 25%">{{__('tender.schedule.fields.end_date', ['type' => __('tender.schedule_type_label.commercial')])}}</th>
                        <td class="text-center" style="width: 2%">:</td>
