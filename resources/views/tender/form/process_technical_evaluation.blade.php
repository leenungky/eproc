...
$inWorkflowValues = [$type];
if($tender->prequalification != 1){
    $inWorkflowValues = ['process_registration',$type];
}

if(in_array($workflowValues[0], $inWorkflowValues)){
    // kondisi halaman technical baru dimulai (submission belum di start)
    if((in_array($workflowValues[0], $inWorkflowValues) && empty($workflowValues[$actIndex]))){
        ...
        $docTabEnable = false;
        $evalTabEnable = false;
    // kondisi halaman technical submission sudah di start atau resubmission
    ...
    // kondisi halaman technical submission sudah di open atau reopen
        ...
        $docTabEnable = true;
        $evalTabEnable = true;
    // kondisi halaman technical submission sudah finish
...
$enableBtnFinish = $statusProcess == 'opened-3';
// dd($statusProcess);
@endphp

@if($isVendor)
    @include('tender.form.technical_evaluation.vendor')
@else
    @include('tender.form.technical_evaluation.admin')
@endif

@section('modals')
@if($isVendor == false)
    @include('tender.form.technical_evaluation.modal_evaluation_note')
    @include('tender.form.technical_evaluation.modal_scoring')
@endif
@include('tender.form.technical_evaluation.modal_comments')
@include('tender.form.technical_evaluation.modal_history')
