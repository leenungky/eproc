<table class="table table-sm table-striped table-bordered">
    ...
    <tbody>
            ...
            @foreach ($profiles as $profile)
                @if($profile->is_current_data)
                    ...
                    @foreach ($profiles as $newdata)
                        @if($profile->id == $newdata->parent_id)
                            <?php $i++; ?>
                            @include('applicants.profiles.partials.finance_detail', ['current'=>$profile, 'new'=>$newdata])
                            <?php $i++; ?>
                            @break;
                        @else
                            @if ($loop->last)
                                @include('applicants.profiles.partials.finance_detail', ['current'=>$profile, 'new'=>null])
                                <?php $i++; ?>
                            @endif
                        @endif
                    @endforeach
                @else
                    @if($profile->parent_id == 0)
                        @include('applicants.profiles.partials.finance_detail', ['current'=>null, 'new'=>$profile])
                        <?php $i++; ?>
                    @endif
                @endif        
            @endforeach        
        @else
            @php ($i=1)
            @include('applicants.profiles.partials.finance_detail', ['current'=>null, 'new'=>null])
        @endif
    </tbody>
</table>
