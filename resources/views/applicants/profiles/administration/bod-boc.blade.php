...
<div class="card-header-right">
    <div class="button-group">
        ...
        @endif
        <button id="btn_create_new_general" class="btn btn-sm btn-success" data-toggle="modal" data-target="#frmbocbod_modal" data-backdrop="static" data-keyboard="false"><i class="fas fa-plus mr-2" aria-hidden="true"></i>{{ __('homepage.create_new_entry') }}</button>
    </div>                        
</div>
@endsection
...
<table class="table table-sm table-striped table-bordered">
    ...
    <tbody>
        @php ($fields = ['board_type','is_the_person_listed_as_company_shareholders','fullname','nationality','email', 'phone_number', 'company_head'])
        @if( $profiles->count() > 0 )
            <?php $i = 1; ?>
...
$modal1 = [
    ...
    'contents' => '',
    'form_layout' => 'applicants.profiles.form.form_bocbod',
    'form_name' => 'frmbocbod',
        ]
?>
...
<script type="text/javascript">
    require(["bootstrap-fileinput"], function () {
        ...
        $('#frmbocbod-save').click(function () {
            var frmData = $('#frmbocbod').serializeArray();
            //frmData.push({name: 'items', value: JSON.stringify(selectedData)});
            $('#frmbocbod_fieldset').attr("disabled", true);
            $.ajax({
    ...
    //                    showAlert("Draft Tender message saved.", "danger", 3000);
                    $('#frmbocbod_fieldset').attr("disabled", false);
                }
