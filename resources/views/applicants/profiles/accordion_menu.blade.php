...
<div class="accordion">
    ...
    <div class="card">
        ...
        <div id="collapse{{$page}}" class="collapse {!! in_array(Request::segment(3),$keys) ? 'show' : '' !!}" aria-labelledby="heading{{$page}}" data-parent=".accordion">
            <div class="">
                <ul class="vertical-nav-menu">
                    @foreach($subpages as $url=>$language)
                    <li><a class="btn btn-link {!! Request::segment(3) === $url ? 'mm-active' : '' !!}" href="{{ route('profile.edit-detail', $url)}}">{{ __('homepage.'.$language) }}</a></li>
                    @endforeach
                </ul>
            </div>
        </div>
