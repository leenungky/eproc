...
<div class="card-header-left">
    <span class="heading-title"><i class="fa fa-list mr-1"></i> Purchase Order Detail</span>
</div>

<div class="card-header-right">
    <div class="button-group">

        <button class="btn btn-sm btn-primary btn-cancel"
            data-keyboard="false">Back</button>
        ...
        @if ($is_show)
            @if ($isEditable && !$isVendor)
                ...
                <button id="btn-submit" class="btn btn-sm btn_submit btn-success" href="{{ route('tender.show', ['id' => $id, 'type' => $type, 'action' => 'detail-specification']) }}">
                    <i class="fa fa-paper-plane"></i>&nbsp;&nbsp;&nbsp;{{__('tender.submit_sap')}}
                </button>
            @endif
        @endif
    </div>
...
<div class="card-fixed" style="padding: 7px 5px 5px;">
    @if (session('status'))
                                ...
                                <table class="table table-borderless">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="td-value">
                                                    <select id="incoterms" name="incoterms" required="required" class="custom-select custom-select-sm"  disabled>
                ...
                columns: [
                    ...
                    @foreach ($tenderData['tender_'.$type]['fields'] as $field)
                        ...
                        @elseif ($field=="delivery_date"){
                            data: '{{$field}}', name: '{{$field}}',
                            "render": function ( data, type, row ) {
                                return '<div class="col-sm-12" style="padding: 0px;"><input id="valid_date-' + row.id  + '" val-id="' + row.item_id  + '" class="form-control form-control-sm datetimepicker-input" value="' + data + '"  readonly data-toggle="datetimepicker" data-target="#valid_date-' + row.id  + '"></div>';
                            },
                        },
                        @elseif (in_array($field,["total","est_unit_price","overall_limit"])){
                            data: '{{$field}}', name: '{{$field}}',class: 'text-right',
                            "render": function ( data, type, row ) {
                                return '<span class="{{$field}}">' + formatAmmount(data, getCurrencyCode()) + '</span>';
                            },
                        },
                        @else
                            {data: '{{$field}}', name: '{{$field}}'},
        ...
        openDetailRow : function(dtrow){
            ...
            for(let ix in fields){
                ...
                @if($editable)
                    ...
                    if((arrEdit.includes(fields[ix]) && dtrow.item_category == 0)){
